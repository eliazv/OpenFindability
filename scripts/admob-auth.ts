...
const PORT = Number(process.env.ADMOB_AUTH_PORT ?? 53682);
const REDIRECT_URI = `http://localhost:${PORT}/oauth2callback`;
const SCOPES = ["https://www.googleapis.com/auth/admob.readonly", "https://www.googleapis.com/auth/admob.report"];

async function main() {
  const clientId = process.env.ADMOB_CLIENT_ID?.trim();
  const clientSecret = process.env.ADMOB_CLIENT_SECRET?.trim();
  if (!clientId || !clientSecret) {
    throw new Error("Set ADMOB_CLIENT_ID and ADMOB_CLIENT_SECRET in .env before running this script.");
  }
  ...
  console.log("Open this URL and sign in with the Google account that owns the AdMob account:");
  console.log();
  console.log(authUrl);
  console.log();
